...
export function MissionCheckInModal({ open, onOpenChange, mission, onSubmit }: MissionCheckInModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const startKm = mission.kmDepart || 0;
        if (kmRetour && kmRetour >= startKm) {
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] p-0 border-none rounded-2xl shadow-xl bg-slate-50 dark:bg-slate-950 flex flex-col overflow-hidden">
                <div className="shrink-0 px-6 py-4 bg-fleet-blue text-white flex items-center justify-between sticky top-0 z-50">
                    <DialogTitle className="text-xl font-black tracking-tight flex items-center gap-2">
                         ...
                         Enregistrement du Retour (Check-In)
                    </DialogTitle>
                    <DialogDescription className="sr-only">
                        Enregistrement du retour pour la mission.
                    </DialogDescription>
                ...
                <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                    <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-6">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-6">
                                <div className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center justify-between">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-sm font-black text-emerald-600 uppercase">{(mission.kmDepart || 0).toLocaleString('fr-FR')} KM</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    {kmRetour !== '' && mission.kmDepart && kmRetour >= (mission.kmDepart || 0) && (
                                        <div className="text-[10px] font-black text-emerald-600 uppercase tracking-widest text-right mt-1">
                                            +{(Number(kmRetour) - (mission.kmDepart || 0)).toLocaleString('fr-FR')} KM PARCOURUS
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                    {mission.typeCarburantDotation !== 'BON' && (
                                        <div className="space-y-4">
                                            ...
                                            {(mission.typeCarburantDotation === 'CARTE' || montantCarburant !== '') && (
                                                <div className="space-y-1.5">
                                    ...
                                    {mission.typeCarburantDotation === 'BON' && (
                                        <div className="flex flex-col items-center justify-center py-4 text-center">
                                            <Fuel className="w-8 h-8 text-amber-500 mb-2 opacity-50" />
                                            <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest">Dotation par BON</p>
                                            <p className="text-[9px] font-bold text-amber-600/70 italic uppercase">La consommation est automatiquement calculée selon la valeur du bon.</p>
                                        </div>
                                    )}
                    ...
                    <DialogFooter className="shrink-0 p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3 px-6">
                        ...
                        <Button type="submit" className="h-9 rounded-xl px-10 font-bold bg-fleet-blue hover:bg-fleet-blue-dark shadow-lg shadow-fleet-blue/20 text-[11px] text-white uppercase flex items-center gap-2" disabled={!kmRetour || (mission.kmDepart && Number(kmRetour) < mission.kmDepart) || !observation}>
                            VALIDER LE RETOUR
